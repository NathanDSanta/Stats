...
int *crearVector(int mida, int grauDeDesordre, bool invers = false) {
  ...
  for (int i = 0; i < grauDeDesordre * (2 * mida / N_VECTORS); i++)
    intercambiar_random(vector, mida);
...
double calcularCoeficientOrdenacio(const int arr[], int mida) {
  ...
  for (int i = 0; i < mida * PERCENT_ITERACIONS_COEFICIENT; i++) {
    ...
    if (a > b && arr[a] > arr[b])
      n_ordenats++;
    else if (a < b && arr[a] < arr[b])
      n_ordenats++;
  }
  return (n_ordenats / (mida * PERCENT_ITERACIONS_COEFICIENT));
}

void Log(int mida, int grauDesordre, int algoritme, int experiment, int durada,
         double coeficient) {
  cout << "Log: ";
  cout << ALGORITMES[algoritme] << " Mida: " << MIDES[mida]
       << " Iteracions: " << grauDesordre * (2 * MIDES[mida] / N_VECTORS)
       << " Experiment nr: " << experiment << " Temps: " << durada
...
DataFrame createRTables(int ****durations, double ****coeficientsAleatorietat) {
  IntegerVector mides;
  NumericVector coeficient;
  CharacterVector algoritme;
    ...
    for (int j = 0; j < N_VECTORS; j++) {
      for (int k = 0; k < N_ALGORITMES; k++) {
        for (int l = 0; l < N_EXPERIMENTS; l++) {
          mides.push_back(MIDES[i]);
          coeficient.push_back(coeficientsAleatorietat[i][j][k][l]);
          algoritme.push_back(ALGORITMES[k]);
          experiments.push_back(l + 1);
          temps.push_back(durations[i][j][k][l]);
        }
      }
    }
  }

  return DataFrame::create(Named("Mida") = mides,
                           Named("Coeficient Ordenacio") = coeficient,
                           Named("Algoritme") = algoritme,
...
DataFrame createTables() {
    ...
    for (int j = 0; j < N_VECTORS; j++) {
      for (int k = 0; k < N_ALGORITMES; k++) {
        for (int l = 0; l < N_EXPERIMENTS; l++) {
          ...
          coeficientAleatoritat[i][j][k][l] =
              calcularCoeficientOrdenacio(arr, MIDES[i]);
          durationsAll[i][j][k][l] =
              test_algorithm(arr, MIDES[i], callbacks[k]);
          Log(i, grauDesordre, k, l, durationsAll[i][j][k][l],
              coeficientAleatoritat[i][j][k][l]);
          delete[] arr;
        }
      }
  ...
  return createRTables(durationsAll, coeficientAleatoritat);
}
